...
const { authenticateToken, autherizeAdmin } = require('../middleware/auth');
...
// if (typeof autherizeAdmin !== 'function') throw new Error('autheriseAdmin is not a function');
...
router.get('/google/callback', passport.authenticate('google', { failureRedirect: '/' }), async (req, res) => {
  try {
    ...
    if (state.type === 'signup') {
      if (user.mobile && user.address1 && user.city && user.state && user.country && user.pincode) {
...
router.post('/complete-profile', authenticateToken, async (req, res) => {
  ...
  try {
    ...
    console.log('New token generated and set in cookie:', token);
...
const adminEmails = ['ras535212@gmail.com', 'anotheradmin@example.com']; // Replace with your actual admin emails
